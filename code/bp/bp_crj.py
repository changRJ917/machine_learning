...
class BPNetwork(object):
    ...
    def train(self, inputs_org, groundtruth_org):
        ...
        inputs = np.array(inputs_org, ndmin=2).T  # input.shape=(784,1)
        groundtruth = np.array(groundtruth_org, ndmin=2).T  # groundtruth.shape=(64,1)
        # 前向传播
        hid_ints = np.dot(self.w_in2hid, inputs)  # hid_ints.shape=(32,1)
        hid_outs = self.act_func(hid_ints)  # hid_outs.shape=(32,1)
        # 输出层（激活函数设置为f(x) = x）
        out_ints = np.dot(self.w_hid2out, hid_outs)  # out_ints.shape=(64,1)
        out_outs = out_ints  # out_outs.shape=(64,1)
        # 反向传播
        out_error = out_outs - groundtruth # out_error.shape=(64,1)
        hid_error = np.dot(out_error.T, self.w_hid2out) * (hid_outs * (1-hid_outs)).T
        # hid_error.shape=(1,32)
        # 上式中(1,64)*(64,32)*(1,32)=(1,32)
        # 更新权重
        # 更新w_hid2out
